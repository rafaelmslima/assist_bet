...
type League = { key: string; label: string; league_id: number; season: number };
type Fixture = {
...
type FixtureList = { ok: boolean; date: string; league: League | null; fixtures: Fixture[]; error?: string | null };
...
const tabs = ["Análise IA", "Escalações", "Desfalques", "Jogadores", "Dados"] as const;
...
async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(payload.detail || "Não consegui completar a requisição.");
  }
...
function Dashboard({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  const [activeTab, setActiveTab] = useState<Tab>("Análise IA");
  ...
  async function refreshFixtures() {
    ...
    try {
      const result = await api<FixtureList>(`/api/fixtures?date=${encodeURIComponent(dateMode)}&league_key=${encodeURIComponent(selectedLeague)}`);
      if (!result.ok) {
        setError(result.error || "Não encontrei jogos para esse filtro.");
      }
  ...
  async function selectFixture(fixture: Fixture) {
    ...
    try {
      ...
      setError(exc instanceof Error ? exc.message : "Falha ao gerar análise.");
    } finally {
      ...
      <section className="fixture-column">
        ...
        <div className="fixture-list">
          {fixtures.map((fixture) => (
            ...
            >
              <div>
                <strong>{fixture.home_team || "Mandante"} x {fixture.away_team || "Visitante"}</strong>
      ...
      <section className="detail-column">
        ...
        {!selectedFixture && !error && (
          <div className="empty-detail">
            ...
            <p>Selecione um jogo para abrir a análise.</p>
          </div>
        )}
          ...
          <>
            <header className="detail-header">
              <div>
                ...
                <h2>{selectedFixture.home_team || "Mandante"} x {selectedFixture.away_team || "Visitante"}</h2>
              </div>
            ...
            {loadingAnalysis && <StateLine icon={<Clock size={18} />} text="Gerando dossiê e análise..." />}
...
function DetailTab({ activeTab, payload }: { activeTab: Tab; payload: AnalysisPayload }) {
  if (activeTab === "Análise IA") {
    return <TextBlock text={payload.advisor_text || "Análise indisponível."} />;
  }
  if (activeTab === "Escalações") {
    const lineups = readPath(payload.dossier, ["lineups"]);
    return <JsonBlock data={lineups || { status: "Escalações indisponíveis." }} />;
  }
  if (activeTab === "Desfalques") {
    return <TextBlock text={payload.injuries_text || "Desfalques indisponíveis."} />;
  }
  if (activeTab === "Jogadores") {
    return <TextBlock text={payload.player_advice_text || "Jogadores indisponíveis."} />;
  }
...
function formatFixtureMeta(fixture: Fixture) {
  const parts = [fixture.fixture_date ? new Date(fixture.fixture_date).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" }) : null, fixture.round, fixture.status]
    .filter(Boolean)
    .join(" · ");
